...
MN_formula_list <- list(
  # Fixed Effect
  won~-1+scale(declared_weight)+scale(actual_weight)+implied_pro+f(race_id, model="iid",hyper = list(prec = list(initial = -lprec, fixed=TRUE))),
  won~-1+scale(declared_weight)+scale(actual_weight)+implied_pro+scale(horse_win_rate)+scale(jockey_win_rate)+f(race_id, model="iid",hyper = list(prec = list(initial = -lprec, fixed=TRUE))),
  won~-1+scale(declared_weight)+scale(actual_weight)+implied_pro+scale(horse_win_rate)+scale(jockey_win_rate)+ICA_1+ICA_2+ICA_3+f(race_id, model="iid",hyper = list(prec = list(initial = -lprec, fixed=TRUE))),
  # Random Effect
  won~-1+scale(declared_weight)+scale(actual_weight)+f(horse_id,model="iid")+f(jockey_id,model="iid")+f(trainer_id,model="iid")+f(race_id, model="iid",hyper = list(prec = list(initial = -lprec, fixed=TRUE))),
  won~-1+scale(declared_weight)+scale(actual_weight)+implied_pro+scale(horse_win_rate)+scale(jockey_win_rate)+ICA_1+ICA_2+ICA_3+f(horse_id,model="iid")+f(jockey_id,model="iid")+f(trainer_id,model="iid")+f(race_id, model="iid",hyper = list(prec = list(initial = -lprec, fixed=TRUE)))
...
for (i in seq_along(subsets)){
  ...
  df_pred <- data_pre_process(train_data, test_data, type="MN")
  ...
  for (formula in MN_formula_list){
    # train the model
    # obtain the posterior mean of winning probabilities -- need to write a new function
    MN.model.post = inla(formula,
                         family = "poisson",
                         data=df_pred,
                         control.predictor = list(hyper = list(prec = list(initial = lprec)),compute = TRUE),
                         control.inla = list(tolerance = 1E-12), 
                         control.compute=list(config = TRUE,dic=T,cpo=T,waic=T))
    filename <- paste0("models/MN_model_", set,"_",models, ".rds")
    saveRDS(MN.model.post, file = filename)
    
    type<-"MN"
    models <- models+1
    mean_win_prob[[models]] <- generate_mean_win_probabilities(100,100,type,MN.model.post,df_pred,times)
    
    
  }
  mean_win_prob_df <- data.frame(mean_win_prob)
  write.csv(mean_win_prob_df, paste0("winning probabilities mean/MN_mean_win_prob_df",set,".csv"), row.names = FALSE)
